...
def tally_values(csv_file, column_numbers=False, threshold=10):
	with open(csv_file, "r") as f:
		header=line_to_list( trim_formatting(f.readline()))
		if not column_numbers:
			column_numbers=range(len(header))
		column_names=separate_features(header, column_numbers)[0]
		values_dict={key:[] for key in column_names}
		for line in f:
			line=line_to_list(trim_formatting(line))
			values=separate_features(line, column_numbers)[0]
			values_dict={key:list(set(values_dict[key]+[value])) for key,value in zip(column_names,values)
						if (key in values_dict) and (bool(threshold)== (len(values_dict[key])<threshold) )}
	with open(os.path.splitext(csv_file)[0]+"_vtally.json", "w") as t:
		json.dump(values_dict, t)
...
def consolidate(orig_file, id_names, feature_names=None):
	...
	feat_start=len(id_names)
	repeated_feature_columns={feat:[col] for feat, col in zip(feature_names,range(feat_start, (feat_start+len(feature_names))) )}
...
def make_choice(unique_id, feature_name, values, hist_values=False, initial=True, mode_default=False, underscore=False):
	"""prompts user to choose the value from a list to write to for a value conflict for a given feature """
	##i'd likee to update the hist_values...... maybe?
	mode, mode_rep=mode_of_list(values)
	if mode_default:
		if mode_default not in [0,1]:
			print "Mode Default not recognized. Choose 0 for always, 1 for >.75 %, of False for never"
			return ""
		if len(mode)==1:
			if mode_rep/float(len(values))>=.75 or mode_default==0:
				return mode[0]
			elif underscore:
				return "_".join(sorted(list(set(values))))
			else: 
				return make_choice(unique_id, feature_name, values, hist_values)
		elif underscore:
			return "_".join(sorted(list(set(values))))
		else:
			return make_choice(unique_id, feature_name, values, hist_values)
		...
		print "choose "+ " : ".join([ str(i)+ " for "+ str(val)  for i,val in  zip(range(len(set(values))), set(values))]) +" or N for Null or M for mode or U for userdefined"
		...
		if choice in [str(i) for i in range(len(set(values)))]:
			...
			mode=mode_of_list(values)[0]
			if len(mode)>1:
				print "There is more than one mode:"
				print mode
				return make_choice(unique_id, feature_name, values,hist_values, initial=False)
			else:
				return str(mode[0])
		elif choice in ["U"]:
			if hist_values:
				print "Existing values for" + feature_name
				return make_choice(unique_id, feature_name, values=hist_values, initial=False)
			else:
				print "Pleae provide value"
				first_value=trim_formatting(raw_input("-->"))
				print "Please provide value again"
				second_value=trim_formatting(raw_input("-->"))
				if first_value==second_value:
					return first_value
				else:
					print "Input does not match"
					return make_choice(unique_id, feature_name, values, initial=False)
		else:
			print "INVALID INPUT"
			return make_choice(unique_id, feature_name, values, initial=False)
...
def get_indices(full_list, column_names, partial_match=False):
	...
	if partial_match:
		return {key:[i for i in  range(len(full_list)) if re.match("^"+key+"[_\d*]*$",full_list[i]) ] for key in column_names}
	else: 
...
def check_duplicates(input_file, id_names, feature_names, log_file=None, id_indices= None, feature_indices=None, hist_values=False, mode_default=False, underscore=False):
		...
		with open(res_file, "w") as r: ###open file to write to
			...
			for line in f:
				...
				for feature_name in feature_names:
					...
					if not values:
						value=""
					elif values==[values[0]]*len(values): #check if all values are the same
						...
						value=make_choice(unique_id, feature_name, values, hist_values, mode_default=mode_default, underscore=underscore) #ask user to choose value
...
def console_and_resolve(csv_file, id_names=None, feature_names=None, log_file=None, mode_default=1, underscore=True  ):
	...
	hist_values=tally_values(csv_file)
	check_duplicates(cons_file, id_names, feature_names, log_file, id_col, feat_col, hist_values, mode_default, underscore)